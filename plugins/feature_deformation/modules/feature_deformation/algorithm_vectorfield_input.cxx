...
void algorithm_vectorfield_input::set_input(const algorithm_grid_input& grid, const std::string& array_name)
{
...
{
    if (!this->input_grid.get().is_valid() && this->input_grid.get().get_results().grid->GetPointData()->GetArray(this->array_name.c_str()) != nullptr)
    {
        return -1;
    }

    return jenkins_hash(this->input_grid.get().get_hash(), this->array_name,
        this->input_grid.get().get_results().grid->GetPointData()->GetArray(this->array_name.c_str())->GetMTime());
}
